...
<body>
        ...
        <div class="container">
                    ...
                    <div class="section-heading">
                        <div class="row" method="GET" action="search.php">
                            <span class="col-lg-7 col-sm-7">
                                <h6>Materi</h6>
                                <h2>Kumpulan <em>Materi</em> PMB STAIMA Al Hikam <span>Malang</span></h2>
                            </span>
                            <div class="col-lg-5 col-sm-5">
                                <form action="{{ route('search3') }}" method="GET">
                                    <input type="search" name="query3" placeholder="Cari Materi berdasarkan judul"
                                        value="{{ request()->query('query3') }}">
                            ...
                            @auth
                                @if (auth()->user()->role == 'admin')
                                    <div class="main-green-button">
                                        <a href="{{ route('materi.create') }}">Upload Materi Baru</a>
                                    </div>
                                @endif
                            @endauth
                        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-4 col-sm-4">
                            <div class="about-item">
                                <h6><b><a href="{{ route('materi.show', $materi->id) }}">{{ $materi->judul }}</a></b>
                                </h6>
                            </div>
                        </div>
                        @auth
                            <div class="col-lg-4 col-sm-4">
                                <div class="about-item">
                                    <div class="main-blue-button">
                                        <a href="{{ route('Admin.materi.edit', $materi->id) }}">Edit</a>
                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-4 col-sm-4">
            ...
            <div class="pagination-wrapper">
                <div class="pagination justify-content-center">
                    {{ $materis->links('pagination::bootstrap-4') }}
                </div>
                <div class="pagination-details">
                    <p>Showing {{ $materis->firstItem() }} to {{ $materis->lastItem() }} of {{ $materis->total() }}
                        results</p>
                </div>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var deleteForms = document.querySelectorAll('form[action*=/materi/]');

            deleteForms.forEach(function(form) {
                form.addEventListener('submit', function(event) {
                    var confirmed = confirm('Are you sure you want to delete this materi?');
                    if (!confirmed) {
